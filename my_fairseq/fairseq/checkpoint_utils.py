...
def get_last_checkpoint_name(args):
    suffix = getattr(args, "checkpoint_suffix", "")
    return "checkpoint_last{}.pt".format(suffix)
...
def save_checkpoint_at_budget(args, trainer, budget, epoch_itr, bleu_score):
    from fairseq import distributed_utils, meters
    ...
    suffix = getattr(args, "checkpoint_suffix", "")
    checkpoint_conds = collections.OrderedDict()
    checkpoint_conds["checkpoint_budget_{}{}.pt".format(budget, suffix)] = True
    ...
    if len(checkpoints) > 0:
        ...
        write_timer.stop()
        logger.info(
            "saved checkpoint {} (budget {} @ {} updates with greedy BLEU {}) (writing took {} seconds)".format(
                checkpoints[0], budget, updates, bleu_score, write_timer.sum
            )
        )