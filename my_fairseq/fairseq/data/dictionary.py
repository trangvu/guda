...
class PrebuiltDictionary(Dictionary):
    def __init__( self):
        self.symbols = []
        self.count = []
        self.indices = {}

    def init_special_tokens(self, pad="[PAD]", eos="[SEP]", unk="[UNK]", bos="[EOS]", extra_special_symbols=None):
        self.unk_word, self.pad_word, self.eos_word = unk, pad, eos
        self.unk_index = self.index(unk)
        self.bos_index = self.index(bos)
        self.pad_index = self.index(pad)
        self.eos_index = self.index(eos)
        special_sets = set([pad, eos, unk, bos])
        if extra_special_symbols:
            for s in extra_special_symbols:
                self.add_symbol(s)
                special_sets.add(s)
        self.nspecial = len(special_sets)