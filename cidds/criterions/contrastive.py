...
class ContrastiveLoss(FairseqCriterion):

    def __init__(self, task, sentence_avg, normalize=True, no_clustering=False):
        super().__init__(task)
        self.sentence_avg = sentence_avg
        self.temperature = task.args.temperature
        self.adaptive_layer_name = task.args.adaptive_layer_name
    ...
    def add_args(parser):
        ...
        parser.add_argument('--no-clustering', action="store_true", default=False,
                            help='Not apply clustered label masking')
        # fmt: on
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        assert (
          hasattr(model, 'adaptive_layers')
          and self.adaptive_layer_name in model.adaptive_layers
        ), 'model must provide sentence classification head for --criterion=sentence_prediction'
        net_output = model(**sample['net_input'],
                         features_only=True,
                         adaptive_head_name=self.adaptive_layer_name
                         )
        ...
        if self.normalize:
          src_rep = F.normalize(src_rep, p=2, dim=-1)
          tgt_rep = F.normalize(tgt_rep, p=2, dim=-1)
        ...
        # # use labels to create masks
        domain_labels = sample['label'].unsqueeze(0).repeat(bsz,1)
        num_sample = sample['nsentences'] * 2
        num_sents = sample['nsentences'] * 2
        if self.no_clustering:
            loss, _ = self.compute_loss(logit, labels, reduce=reduce)
        else:
            label_masks = domain_labels.eq(domain_labels.transpose(1,0)).int()
            label_masks = label_masks.fill_diagonal_(1)
            label_masks = torch.cat([label_masks, label_masks], 0).to(device)
            label_masks = torch.cat([label_masks, label_masks], 1).to(device)
            loss, _ = self.compute_loss(logit, labels, mask=label_masks, reduce=reduce)
    ...
    def compute_loss(self, logit, target, mask=None, reduce=True):
