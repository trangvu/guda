...
class PretrainedDictionary(Dictionary):
    """A mapping from symbols to consecutive integers"""
    ...
    def __init__(
        ...
        *,  # begin keyword-only arguments
        ...
        if extra_special_symbols:
            for s in extra_special_symbols:
                self.add_symbol(s)
        self.nspecial = 4 + len(extra_special_symbols) if extra_special_symbols is not None else 0

    def bos(self):
        """Helper to get index of beginning-of-sentence symbol"""
        if self.bos_index is None:
...
class FBXLMDictionary(Dictionary):
    """A mapping from symbols to consecutive integers"""
