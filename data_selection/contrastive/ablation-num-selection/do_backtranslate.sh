...
src_models=( ${src_dataset}-from_en )
src_langs=( en )
tgt_langs=( $L )
...
for ((j=0;j<${#src_models[@]};++j)); do
    ...
    fairseq-generate $DATA_DIR \
        ...
        --sampling --beam 1  >  $tmp/$log_name.log

    echo "Extract BT data and save to "$SAVE_DIR/bt_$f
    python3 ./extract_bt_data.py \
    --minlen 1 --maxlen 250 --ratio 1.5 \
    --output $SAVE_DIR/bt_$f --srclang $TGT_LANG --tgtlang $SRC_LANG \
    $tmp/$log_name.log
done
