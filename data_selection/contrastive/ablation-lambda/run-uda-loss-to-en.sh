...
SAVE_DIR=$ALNMT_DIR/models/ablation-uda-pseudo/$DATASET/uda-to-en-lamda-$lambda1-$lambda2-$lambda3
echo '**************************************'
...
fairseq-generate $EVAL_DIR \
	  --path $SAVE_DIR/checkpoint_last.pt \
	    --task da_translation \
	      --user-dir $USR_DIR \
	        --source-lang $SRC_LANG --target-lang $TGT_LANG \
		  --beam 5 --remove-bpe=sentencepiece --lenpen 0.6 | tee $SAVE_DIR/test_last.log

grep ^H $SAVE_DIR/last_best.log | cut -f3 | sacremoses detokenize > $SAVE_DIR/test_last.detok.sys
grep ^T $SAVE_DIR/last_best.log | cut -f2 | sacremoses detokenize > $SAVE_DIR/test_last.detok.ref
fairseq-score --sys $SAVE_DIR/test_last.detok.sys --ref $SAVE_DIR/test_last.detok.ref --sacrebleu | tee $SAVE_DIR/score_last.log
cat $SAVE_DIR/test_last.detok.sys | sacrebleu -w 2 $SAVE_DIR/test_last.detok.ref | tee $SAVE_DIR/score_last.log
