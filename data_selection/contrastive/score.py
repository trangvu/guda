...
def run_batched_inference(args, model, tensor_dataset, device, classification_head_name, contrastive=False, batch_size=100):
    ...
    with torch.no_grad():
      ...
      for i in range(len(score)):
        ...
        if predictions[i] == 1:
          ...
          tok = tokenizer.convert_ids_to_tokens(batch[0][i][:seq_len[i]])
          detok = ' '.join(tok).replace(' ##', '')
          score_output.append((idx, 1 - score[i], detok))
...
def main(args, **unused_kwargs):
    ...
    if bexists:
      ...
      logger.error("Cannot file checkpoint {}".format(args.model_path))
      raise
    ...
    start = time.time()
    run_batched_inference(args, model, tensor_dataset,
                          device, model_args.classification_head_name,
                          contrastive=(model_args.task == 'sentence_prediction_from_contrastive_bert'),
