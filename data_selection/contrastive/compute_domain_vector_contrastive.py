...
from fairseq import options, tasks, checkpoint_utils, utils
from fairseq.file_io import PathManager
from torch.serialization import default_restore_location

from nmt.acl2021.cidds.tasks.constrastive_nmt_from_pretrained_bert import ContrastiveNMTFromPretrainedBertTask
...
def run_batched_inference(args, tensor_dataset, batch_size=100):
  ...
  vocab_dict = ContrastiveNMTFromPretrainedBertTask.load_dictionary(args.vocab)
  task = ContrastiveNMTFromPretrainedBertTask(args, vocab_dict, vocab_dict, None)
  ...
  for batch in tqdm(inference_dataloader, desc="Running inference..."):
    ...
    with torch.no_grad():
      ...
      outputs = model(**inputs)['src_output']  # of shape [batch_size, seq_len, state_size]
...
def encode_text_file_and_save(args, file_path, output_path, max_lines_to_encode, sentencepience=False):
  ...
  start = time.time()
  avg_pooled = run_batched_inference(args, tensor_dataset, batch_size=128)
  end = time.time()
...
def main():
  ...
  langs = ["de", "fr", "cs"]
  domains = ["law", "ted", "med", "it", "koran"]
  for lang in langs:
    for domain in domains:
      target_langs = ["en", lang]
      for tgt in target_langs:
        print("Process {} {} {}".format(lang, domain, tgt))
        args.model_path = "/home/vuth0001/workspace/acl2021/visualization/contrastive/cons_en-{}-k5/checkpoint_best.pt".format(lang)
        args.input = "/home/vuth0001/workspace/acl2021/visualization/en-{}/{}/valid.{}".format(lang, domain, tgt)
        args.output = "/home/vuth0001/workspace/acl2021/visualization/en-{}/{}/{}_cons_prep_k5.npy".format(lang, domain, tgt)
        ...
        encode_text_file_and_save(args, input_file, output_file, max_lines_to_encode, sentencepience=args.sentencepiece)

  langs = ["ar"]
  domains = ["ted", "it", "koran"]
  for lang in langs:
    for domain in domains:
      target_langs = ["en", lang]
      for tgt in target_langs:
        ...
        args.model_path = "/home/vuth0001/workspace/acl2021/visualization/contrastive/cons_en-{}-k5/checkpoint_best.pt".format(lang)
        args.input = "/home/vuth0001/workspace/acl2021/visualization/en-{}/{}/valid.{}".format(lang, domain, tgt)
        args.output = "/home/vuth0001/workspace/acl2021/visualization/en-{}/{}/{}_cons_prep_k5.npy".format(lang, domain,
                                                                                                        tgt)
